...
public class MainHyperparameterSearchSARScan {

    public static void main (String [] args) {

        System.out.println("Hyperparameter Search SARSA");
                ...
                for (double epsilon : epsilonArray) {
                    ...
                    for (Unit unit : studyMap.getUnits()) {
                        if (studyMap.getUnitsScanStatisticsEvaluation().get(unit.getId()) != -1.0) {
                            SARSA sarsa = new SARSA(studyMap, unit, episodes, steps, alpha, gamma, epsilon,
                                    false);
                            sARScan.add(sarsa);
                        }
                    }
                    ...
                    int index = SARSA.returnIndexBestEnvironment(sARScan);
                    ...
                    System.out.printf("improvement: %d ", sARScan.get(index).getImprov());
                    System.out.println();
                    i++;
