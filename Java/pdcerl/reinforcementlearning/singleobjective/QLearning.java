...
// -----------------------------------------------------------------------------
//  Q-Learning for Irregular Spatial Cluster Detection
// -----------------------------------------------------------------------------

public class QLearning implements IConstant {
    ...
    public QLearning (StudyMap studyMap, Unit unit, int episodes, int steps, double alpha, double gamma, double epsilon,
                      boolean verbose) {
    ...
    public void training() {
        ...
        for (int episode = 0; episode < this.episodes; episode++) {
            ...
            for (int step = 1; step < this.steps + 1; step++) {
                ...
                this.q[state][action] = this.q[state][action] + this.alpha * (this.reward + this.gamma
                        * (Math.max(this.q[nstate][0], this.q[nstate][1])) -this.q[state][action]);
    ...
    public static Solution returnBestEnvironment(List<QLearning> qLearningList) {

        double maxValue = 0.0;
        int index = 0;
        int indexMaxValue = 0;

        for (QLearning qLearning: qLearningList) {
            if (qLearning.getEnvironment().getScanStatisticsValue() > maxValue) {
                maxValue = qLearning.getEnvironment().getScanStatisticsValue();
                indexMaxValue = index;
            }
            index++;
        }
        ...
        solution.copy(qLearningList.get(indexMaxValue).getEnvironment());

        return solution;
    }

    public static int returnIndexBestEnvironment(List<QLearning> qLearningList) {
