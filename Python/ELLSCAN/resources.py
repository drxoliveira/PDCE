...
class Resource:
    ...
    @staticmethod
    def return_best_solution(results=list()):

        best_solution = Solution(list(), .0, .0)

        for result in results:        
            if best_solution.get_scan_statistics_value() < result.get_environment().get_scan_statistics_value():            
                best_solution.copy(result.get_environment())
    ...
    @staticmethod
    def plot_zone(zone=list(), study_map=StudyMap(), name="cluster", save_figure=False):
    ...
    @staticmethod
    def execute_training_test(algorithm):
